...
export default function NewMail() {
    ...
    const handleSubmit = async (e) => {
        ...
        const mail = { numArrive, dateArrivee, numDepartExp, dateDepartExp, expediteur, objet, numReponse }
        const url = 'http://localhost:8080/mail/api/mail/upload';
        console.log(files)
        console.log(mail)
        ...
        formData.append('mail', new Blob([JSON.stringify(mail)], { type: 'application/json' }));
    ...
    return (
        <div className="bg-slate-600 relative flex flex-col justify-center min-h-min overflow-clip">
            ...
            <div className="mt-4 mb-4 w-full p-6 mx-auto bg-white rounded-md shadow-xl lg:max-w-xl">
                ...
                <p ref={errRef} className={errMsg ? "errmsg uppercase text-center bg-red-500 text-white px-4 py-2" : "offscreen"} aria-live="assertive">{errMsg}</p>
                {success ?
                    <div className="p-4 mb-4 text-sm text-green-700 bg-green-100 rounded-lg dark:bg-green-200 dark:text-green-800 duration:2000" role="alert">
                        <p className="font-medium text-center text-lg">Courrier enregistré avec succès</p>
                    </div> : ''

                }
                <form className="mt-2" onSubmit={handleSubmit}>
                    <div className="mb-2">
                        <label className="block text-sm font-semibold text-gray-800" >Numéro</label>
                        <input
                    ...
                    {numArriveError ? <p style={{ color: 'red', fontSize: '14px' }}>{numArriveError}</p> : ''}

                    <div className="mb-2">
                        <label className="block text-sm font-semibold text-gray-800" > Date d'arrivée</label>
                        <input
                            type="date"
                            required
                        ...
                        />
                        {dateArriveeError ? <span style={{ color: 'red', fontSize: '14px' }}>{dateArriveeError}</span> : ''}

                    </div>
                    <div className="mb-2">
                        <label className="block text-sm font-semibold text-gray-800" >Numéro de départ</label>
                        <input
                            type="number"
                            required
                            className="font-semibold h-8 block w-full px-4 py-2 mt-0 text-purple-700 bg-white border rounded-md focus:border-purple-400 focus:ring-purple-300 focus:outline-none focus:ring focus:ring-opacity-40"
                        ...
                        />
                        {numDepartExpError ? <span style={{ color: 'red', fontSize: '1214px' }}>{numDepartExpError}</span> : ''}

                    </div>

                    <div className="mb-2">
                        <label className="block text-sm font-semibold text-gray-800" >Date de départ</label>
                        <input
                            type="date"
                            required
                            className="font-semibold h-8 block w-full px-4 py-2 mt-0 text-purple-700 bg-white border rounded-md focus:border-purple-400 focus:ring-purple-300 focus:outline-none focus:ring focus:ring-opacity-40"
                        ...
                        />
                        {dateDepartExpError ? <span style={{ color: 'red', fontSize: '14px' }}>{dateDepartExpError}</span> : ''}

                    </div>
                    <div className="mb-2">
                        <label className="block text-sm font-semibold text-gray-800 " >Expediteur</label>
                        <input
                            type="text"
                            required
                            className="font-semibold h-8 block w-full px-4 py-2 mt-0 text-purple-700 bg-white border rounded-md focus:border-purple-400 focus:ring-purple-300 focus:outline-none focus:ring focus:ring-opacity-40"
                        ...
                        />
                        {expediteurError ? <span style={{ color: 'red', fontSize: '14px' }}>{expediteurError}</span> : ''}

                    </div>
                    <div className="mb-2">
                        <label className="block text-sm font-semibold text-gray-800" >Objet</label>
                        <input
                            type="text"
                            required
                            className="font-semibold h-8 block w-full px-4 py-2 mt-0 text-purple-700 bg-white border rounded-md focus:border-purple-400 focus:ring-purple-300 focus:outline-none focus:ring focus:ring-opacity-40"
                        ...
                        />
                        {objetError ? <span style={{ color: 'red', fontSize: '14px' }}>{objetError}</span> : ''}

                    </div>
                    <div className="mb-2">
                        <label className="block text-sm font-semibold text-gray-800" >Numéro reponse</label>
                        <input
                            type="number"
                            className="font-semibold h-8 block w-full px-4 py-2 mt-0 text-purple-700 bg-white border rounded-md focus:border-purple-400 focus:ring-purple-300 focus:outline-none focus:ring focus:ring-opacity-40"
                            placeholder=""
                    ...
                    <div>
                        <input style={{ color: 'brown' }} type='file'
                            onChange={handleChange} multiple required className='h-8 block w-full text-sm text-slate-500
