...
function MatTable() {
  ...
  const URL = `http://localhost:8080/mail/api/mail/download/${fichier}`;
  ...
  async function download() {
    ...
    try {
      ...
      const url = window.URL.createObjectURL(new Blob([response.data], { type: "application/zip" })); // you can mention a type if you wish
  ...
  useEffect(() => {
    MailService.getAllMail().then(response => {
      ...
      if (Error.response?.status === 403) {
        ...
        setErrMsg('Erreur ' + Error.response?.status + ' : Autorisation requise');
      }
      if (Error.response?.status === 401 ) {
        console.log(Error.response?.status)
        setErrMsg('Erreur ' + Error.response?.status + ' : authentification requise');
      }
       else {
        setErrMsg('No server response');
      }
  ...
  const columns = [
    ...
    { title: "Fichier", field: "file", align: "center", render: rowData => <Button style={{ height: 25, mt: 4, color: "blue" }} onFocus={() => setFichier(rowData.file)} onClick={download} ><CloudDownloadIcon /></Button> },
  ...
  return (
    <>
      ...
      <div className='bg-slate-600 mx-2 my-4 h-full' >
        <p className={errMsg ? "errmsg uppercase text-center bg-red-500 text-white font-bold px-4 py-2" : "offscreen"} aria-live="assertive">{errMsg}</p>
        <div>
          <ThemeProvider theme={defaultMaterialTheme}>
            <div>
              <MaterialTable
                icons={tableIcons}
                columns={columns} data={mail}
                title='' px
                localization={{
                  pagination: {
                    labelRowsPerPage: '',
                ...
                options={{
                  ...
                  filtering: false,
                  paging: true, paginationType: 'stepped',
                  pageSize: 10,
                  pageSizeOptions: [5, 10, 15, 20, mail.length],
                  exportButton: true,
                  headerStyle: { background: "#838996", fontWeight: "bold", fontSize: "14px", color: "white" },
                  rowStyle: (_data, index) => index % 2 === 0 ? { background: "#eeeef1", fontWeight: "bold", fontSize: '14px', color: "#7777f5" } : { background: "#FBFBF9", fontWeight: "bold", fontSize: '14px', color: "#747475" },
                }}
